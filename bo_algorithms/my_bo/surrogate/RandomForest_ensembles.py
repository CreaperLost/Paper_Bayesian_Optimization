...
class  Ensemble_RF(BaseModel):
    def __init__(self, config_space, rng=None,n_estimators= 100,box_cox_enabled = None):
        ...
        print(f'My ensemble local random forest is deterministic by using seed : {rng,self.rng}')

        self.n_estimators =  n_estimators
        self.standardize_output = box_cox_enabled
        self.models = []
        super(Ensemble_RF, self).__init__()
    ...
    def predict(self, X:np.ndarray):
        ...
        return np.mean(mean_list,axis = 0).reshape([-1,1]), np.mean(var_list, axis =0 ).reshape([-1,1])
