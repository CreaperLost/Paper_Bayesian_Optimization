...
def get_probs_per_configuration(path_to_config_files):
    ...
    for config_prob in configs:
        ...
        #print(f'Got probabilities for {file_name} and config number {config_prob}')
        if new_pd.shape[1] < 3 :
            new_pd = new_pd['1']
    ...
    for data_id in probs_dictionary:
        ...
        for seed in probs_dictionary[data_id]:
            ...
            for fold in probs_dictionary[data_id][seed]:
                if isinstance(probs_dictionary[data_id][seed][fold],list):
                    df_to_concat = turn_multiclass_list_into_pd(probs_dictionary[data_id][seed][fold])
                else:
                    df_to_concat = probs_dictionary[data_id][seed][fold]
...
for data_id in datasets:
    ...
    for opt in OPTIMIZERS:
        ...
        #print(holdout_scores_per_dataset[data_id])
        ...
        corrected_score = 0
        for i in N_SEEDS:
            print(f"Are two fold memberships equal? {fold_membership[data_id][i] == fold_membership_labels[data_id][i]}")
            seed_score = bbc(np.array(config_matrix[data_id][i]),np.array(labels_matrix[data_id][i]),
                            'classification',
                            fold_membership[data_id][i],
        ...
        #print(labels_CV_df[data_id])

        #print(CV_scores_per_dataset[data_id])
